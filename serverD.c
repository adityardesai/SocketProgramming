  ...
  void displayPeerMessage(int sockfd)
  {
      ...
      printf("\nThe serverD has received the network topology from the Client with UDP port number %d and IP address %s", (int)ntohs(peer.sin_port),inet_ntoa(peer.sin_addr)); 
  }
  ...
  char *readfile(char *filename)
  {
      ...
      if((strcmp(filename,"serverD.txt")) == 0)
      {
    ...
    void displayMessage(int sockfd,int check)
    {
      ...
      {  
        ...
        if(z == -1)
        {
          perror("getSockName serverD");
        } 
        printf("\nFor this connection with the Client,the Server D has TCP port number %d and IP address %s \n",ntohs(tempClientPortDetails.sin_port),ipstr);
      }
      if(check == 2)
      {  
        int z;
        ...
        if(z == -1)
        {
          perror("getSockName serverUDPD");
        } 
        printf("\nFor this connection with the Client,the Server D has UDP port number %d and IP address %s \n",ntohs(tempClientPortDetails.sin_port),ipstr);
      }
     } 
    ...
    int main(int argc, char *argv[])
    {
      ...
      char filename1[MAX] = "serverD.txt";
     ...
     // printf("TEST MESSAGE and Sending Message : %s %s \n",message,sendMessage);
      ...
      for(p = servinfo; p != NULL; p = p->ai_next) 
        ...
        printf(" The ServerD finishes sending its neighbor information to Client with TCP Port number %s and IP Address %s:\n",PORTCLTCP,ipstr);
          
        if ((sockfd = socket(p->ai_family, p->ai_socktype,
            p->ai_protocol)) == -1) 
          {
            perror("clientTCPD: socket");
            continue;
          }
          if (connect(sockfd, p->ai_addr, p->ai_addrlen) == -1)
          {
          close(sockfd);
          perror("clientTCPD: connect");
          continue;
          }
          break;
        }//end for p=servinfo
        ...
        if (p == NULL) {
          fprintf(stderr, "client clientTCPD: failed to connect\n");
          return 2;
        }
      ...
      close(sockfd);//close the socket*/
        ...
        if ((rvUDP = getaddrinfo("localhost", UDPPORTD, &hintsUDP, &servinfoUDP)) != 0) {
        fprintf(stderr, "getaddrinfoUDPD: %s\n", gai_strerror(rvUDP));
        return 1;
        }
        ...
        for(pUDP = servinfoUDP; pUDP != NULL; pUDP = pUDP->ai_next) {
          if ((sockfdUDP = socket(pUDP->ai_family, pUDP->ai_socktype,
              pUDP->ai_protocol)) == -1) {
            perror("UDP ListenerD: socket");
            continue;
          }
          if (bind(sockfdUDP, pUDP->ai_addr, pUDP->ai_addrlen) == -1) {
            close(sockfdUDP);
            perror("UDP ListenerD: bind");
            continue;
          }
          break;
        }//end for p=servinfoUDP
        if (pUDP == NULL) {
          fprintf(stderr, "UDP ListenerD: failed to bind socket\n");
          return 2;
        }
    ...
    while(1)
    {
        ...
        /*printf("UDP ListenerA: got packet from %s\n",
        ...
        buf[numbytes] = '\0';
        //printf("UDP ListenerD: packet contains \"%s\"\n", buf);
        
        printf("The serverD has received the network topology from the Client\n");
        printf("\n Edge------------Cost");
