//Acts a TCP Server and UDP Client. In reality this is a server, but name here is client
...
//The following function code converts the global array sendingUDPMessage to the global string AdjMatrix.
// this AdjMatrix is sent to all the servers
int sendAdjMatrix()
{
...
// The following funtion calculates the minimum edge weight for MST
...
//The following function prints the Adjacency Matrix
void printMatrix()
{
...
//The following function code process the incoming data
// forms the global string for future prcessing and calculates the cost matrix  
void processData()
{
...
//The following function displays the details of the node, which is on the other end.
...
//The following functon displays the UDP message that si sent to all the servers
...
//The followinf function code will display the details of the ports used by the client to send data to
//servers
...
/*RE_USE THE PORTS*/
        ...
        //The following function will fail on Nunki for server B and gets exited
        ...
        //Free all the nodes and closes the socket
