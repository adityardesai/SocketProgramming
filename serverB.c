  ...
  char *readfile(char *filename)
  {
      ...
      if((strcmp(filename,"serverB.txt")) == 0)
      {
        printf("\n The Server B is up and running\n The Server B has the following neighbor information:");
        printf("\n Neighbour  Cost\n");
        strcat(message,"B");
        strcat(message,"+");
  ...
  void displayPeerMessage(int sockfd)
  {
      ...
      printf("\nThe serverB has received the network topology from the Client with UDP port number %d and IP address %s", (int)ntohs(peer.sin_port),inet_ntoa(peer.sin_addr)); 
  }
  ...
  void displayMessage(int sockfd,int check)
  {
      ...
      {  
        ...
        if(z == -1)
        {
          perror("getSockName serverB");
        } 
        printf("\nFor this connection with the Client,the Server B has TCP port number %d and IP address %s \n",ntohs(tempClientPortDetails.sin_port),ipstr);
      }
      if(check == 2)
      {  
        int z;
        ...
        if(z == -1)
        {
          perror("getSockName serverUDPB");
        } 
        printf("\nFor this connection with the Client,the Server B has UDP port number %d and IP address %s \n",ntohs(tempClientPortDetails.sin_port),ipstr);
      }
     } 
  void formatUDPMessage(char message[MAXDATASIZE])
    ...
    int main(int argc, char *argv[])
    {
      ...
      char filename1[MAX] = "serverB.txt";
      ...
      if ((rv = getaddrinfo("localhost", PORTCLTCP, &hints, &servinfo)) != 0) {
        ...
        printf(" The ServerB finishes sending its neighbor information to Client with TCP Port number %s and IP Address %s:\n",PORTCLTCP,ipstr);
          ...
          if ((sockfd = socket(p->ai_family, p->ai_socktype,
            p->ai_protocol)) == -1) 
          {
            perror("clientTCPB: socket");
            continue;
          }
          if (connect(sockfd, p->ai_addr, p->ai_addrlen) == -1)
          {
          close(sockfd);
          perror("clientTCPB: connect");
          continue;
          }
            break;
        }//end for p=servinfo
        ...
        if (p == NULL) {
          fprintf(stderr, "client clientTCPB: failed to connect\n");
          return 2;
        }
     ...
     // printf("TCP clientB: connecting to %s\n", s);
      ...
      close(sockfd);//close the socket*/
        ...
        if ((rvUDP = getaddrinfo(NULL, UDPPORTB, &hintsUDP, &servinfoUDP)) != 0) {
        fprintf(stderr, "getaddrinfoUDPB: %s\n", gai_strerror(rvUDP));
        return 1;
        }
        ...
        for(pUDP = servinfoUDP; pUDP != NULL; pUDP = pUDP->ai_next) {
          if ((sockfdUDP = socket(pUDP->ai_family, pUDP->ai_socktype,
              pUDP->ai_protocol)) == -1) {
            perror("UDP ListenerB: socket");
            continue;
          }
          if (bind(sockfdUDP, pUDP->ai_addr, pUDP->ai_addrlen) == -1) {
            close(sockfdUDP);
            perror("UDP ListenerB: bind");
            continue;
          }
          break;
        }//end for p=servinfoUDP
        if (pUDP == NULL) {
          fprintf(stderr, "UDP ListenerB: failed to bind socket\n");
          return 2;
        }
         /* */
        ...
        //Append the \0 to the end of the message
        buf[numbytes] = '\0';
        //printf("UDP ListenerB: packet contains \"%s\"\n", buf);
        printf("The serverB has received the network topology from the Client\n");
        printf("\n Edge------------Cost");
        formatUDPMessage(buf);
       // displayPeerMessage(sockfdUDP);
       
        test=(struct sockaddr_in *)&their_addrUDP;
        printf("\nThe serverB has received the network topology from the Client with UDP port number %d and IP address %s", ntohs(test->sin_port),ipstr); 
